                    ...
                    >
                      ...
                      <div className="group relative max-w-[75%]">
                        <div className={`animate-[springIn_.4s_cubic-bezier(0.34,1.56,0.64,1)] rounded-2xl px-4 py-2.5 text-sm shadow-sm transition-transform duration-200 hover:scale-[1.01] ${mine ? "rounded-br-md bg-navy text-ivory dark:bg-ivory dark:text-navy" : "rounded-bl-md border border-navy/5 bg-white text-slate-700 dark:border-ivory/10 dark:bg-navy/70 dark:text-ivory"}`}>
                          ...
                          {isEditing ? <div className="min-w-52"><input value={editingContent} onChange={(event) => setEditingContent(event.target.value)} className="w-full rounded-lg border border-white/40 bg-white/15 px-2 py-1 text-white outline-none placeholder:text-indigo-100 dark:border-navy/30 dark:bg-navy/10 dark:text-navy dark:placeholder:text-navy/50" autoFocus /><div className="mt-2 flex justify-end gap-2"><button onClick={() => { setEditingMessageId(null); setEditingContent(""); }} className="text-xs text-indigo-100 dark:text-navy/60">Cancel</button><button onClick={() => saveEditedMessage(message._id)} className="rounded-md bg-white/20 px-2 py-1 text-xs font-bold text-white dark:bg-navy/20 dark:text-navy">{isUpdatingMessage ? "Saving..." : "Save"}</button></div></div> : <p className={`break-words leading-relaxed ${message.isDeleted ? "italic opacity-70" : ""}`}>{message.isDeleted ? "This message was deleted" : message.content}</p>}
                          <p className={`mt-1 text-[10px] ${mine ? "text-indigo-100 dark:text-navy/50" : "text-slate-400 dark:text-ivory/40"}`}>{new Date(message.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}{message.editedAt && !message.isDeleted ? " (edited)" : ""}{status && <span className={`ml-1 font-black ${isRead ? "text-sky-300" : "text-indigo-200/70"}`}>{status}</span>}</p>
                        </div>
                        {mine && !message.isDeleted && !isEditing && <div className="absolute -left-20 top-1/2 hidden -translate-y-1/2 gap-1 rounded-lg border border-slate-200 bg-white p-1 shadow-lg group-hover:flex dark:border-navy/30 dark:bg-navy/80"><button onClick={() => startEditingMessage(message)} className="rounded px-2 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-100 dark:text-ivory dark:hover:bg-navy/50">Edit</button><button onClick={() => removeMessage(message._id)} className="rounded px-2 py-1 text-xs font-semibold text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-900/30">Delete</button></div>}
