...
function Register() {
  ...
  return <main className="font-body relative grid min-h-screen place-items-center overflow-hidden bg-ivory px-4 py-8 transition-colors duration-500 dark:bg-navy">
    ...
    <section className="relative w-full max-w-md animate-[authIn_.5s_ease-out] rounded-[2rem] border border-white/70 bg-ivory/80 p-7 shadow-2xl shadow-amber/20 backdrop-blur-xl sm:p-9 dark:border-ivory/10 dark:bg-navy/85 dark:shadow-black/40">
      <div className="mb-7 text-center"><div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 text-2xl text-white shadow-lg shadow-indigo-300">✦</div><p className="text-xs font-black uppercase tracking-[0.24em] text-indigo-500">NovaChat</p><p className="mt-2 text-xs font-black uppercase tracking-[0.24em] text-indigo-500">Join the conversation</p><h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900 dark:text-white">Create account</h1></div>
      <form onSubmit={submitHandler} className="space-y-3.5">
        <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Name<input type="text" placeholder="Your name" value={name} onChange={(event) => setName(event.target.value)} required className="mt-1.5 w-full rounded-2xl border border-navy/10 bg-white/60 px-4 py-3 outline-none transition focus:border-amber focus:ring-4 focus:ring-amber/15 dark:border-ivory/15 dark:bg-navy/60 dark:text-ivory dark:focus:ring-amber/10" /></label>
        ...
        <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Password<input type="password" placeholder="At least 6 characters" value={password} onChange={(event) => setPassword(event.target.value)} required className="mt-1.5 w-full rounded-2xl border border-navy/10 bg-white/60 px-4 py-3 outline-none transition focus:border-amber focus:ring-4 focus:ring-amber/15 dark:border-ivory/15 dark:bg-navy/60 dark:text-ivory dark:focus:ring-amber/10" /></label>
        <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">Confirm password<input type="password" placeholder="Repeat your password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} required className="mt-1.5 w-full rounded-2xl border border-navy/10 bg-white/60 px-4 py-3 outline-none transition focus:border-amber focus:ring-4 focus:ring-amber/15 dark:border-ivory/15 dark:bg-navy/60 dark:text-ivory dark:focus:ring-amber/10" /></label>
